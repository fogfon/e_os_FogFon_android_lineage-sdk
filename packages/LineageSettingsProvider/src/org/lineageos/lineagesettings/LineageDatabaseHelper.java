...
public class LineageDatabaseHelper extends SQLiteOpenHelper{
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if (eUpgradeVersion < 14) {
             if (mUserHandle == UserHandle.USER_OWNER) {
                // Update STATUS_BAR_CLOCK because was centered by db 13 before (not in code anymore)
                db.beginTransaction();
                SQLiteStatement stmt = null;
                try {
                    stmt = db.compileStatement("UPDATE system SET value=? WHERE name=?");
                ...
                // Update STATUS_BAR_BATTERY_STYLE because was displayed as text by db 13 before (not in code anymore)
                db.beginTransaction();
                stmt = null;
                try {
                    stmt = db.compileStatement("UPDATE system SET value=? WHERE name=?");
